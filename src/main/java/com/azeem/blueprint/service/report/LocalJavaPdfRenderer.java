...
public class LocalJavaPdfRenderer implements PdfRenderer {
  ...
  private static final Font HEADING_FONT = new Font(Font.HELVETICA, 13, Font.BOLD, new Color(79, 70, 229));
  ...
  public byte[] render(
    ...
    try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
      ...
      addDepartmentBreakdown(doc, summary.getChargesByDepartment());
  ...
  private void addHeader(Document doc, CorporateInfo info, String billingPeriod)
    ...
    Paragraph meta =
        new Paragraph(
            "Billing Period: " + billingPeriod + "  |  Generated: " + DATE_FMT.format(Instant.now()),
            SMALL_FONT);
  ...
  private void addDepartmentBreakdown(Document doc, Map<String, Double> chargesByDepartment)
      throws DocumentException {
    if (chargesByDepartment == null || chargesByDepartment.isEmpty()) return;

    Paragraph heading = new Paragraph("Charges by Department", HEADING_FONT);
    ...
    List<Map.Entry<String, Double>> sorted =
        chargesByDepartment.entrySet().stream()
            .sorted(Comparator.<Map.Entry<String, Double>, Double>comparing(Map.Entry::getValue).reversed())
            .toList();
  ...
  private void addFooter(Document doc) throws DocumentException {
    ...
    Paragraph footer =
        new Paragraph(
            "Generated by Blueprint Billing Intelligence Platform  |  " + DATE_FMT.format(Instant.now()),
            SMALL_FONT);
