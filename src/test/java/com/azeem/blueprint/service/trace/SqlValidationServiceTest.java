...
class SqlValidationServiceTest {
  ...
  void selectWithAggregations_passes() {
    assertThat(
            validator.isValidSql(
                sql(
                    "SELECT department, SUM(total_charge) FROM billing_records GROUP BY department")))
        .isTrue();
  }
  ...
  void updateStatement_rejected() {
    assertThat(validator.isValidSql(sql("UPDATE billing_records SET total_charge = 0"))).isFalse();
  }
  ...
  void alterStatement_rejected() {
    assertThat(validator.isValidSql(sql("ALTER TABLE billing_records ADD COLUMN x INT"))).isFalse();
  }
